...
class VideoPreviewActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mp.ready(iv_videoView, Uri.fromFile(file), object : MediaPlayerHelper.VideoPlayEventListener {
            override fun onProgress(currentProgress: Int, maxProgress: Int) {
            }

            override fun onStatusChange(status: MediaPlayerHelper.Status) {
                when (status) {
                    MediaPlayerHelper.Status.Start, MediaPlayerHelper.Status.Resume, MediaPlayerHelper.Status.SeekTo -> this@VideoPreviewActivity.play = true
                    MediaPlayerHelper.Status.End, MediaPlayerHelper.Status.Pause -> this@VideoPreviewActivity.play = false
                }
            }
        })
        iv_videoView.useController = false
        iv_media_cancel.setOnClickListener { finish() }
        media_send.setOnClickListener(onSendClick)
        iv_play_button.setOnClickListener(onPlayClick)
    }
...
//        }
    ...
    private fun getSimpleDialog(context: Context, message: String?, title: String?): AlertDialog.Builder {
