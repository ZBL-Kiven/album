 ...
 * 語音播放管理
 */
class VoicePlayManager(context: Context) {
...
//        audioManager.mode = if (isEarpieceMode) AudioManager.MODE_IN_COMMUNICATION else AudioManager.MODE_NORMAL
        audioManager.mode =
            if (isEarpieceMode) AudioManager.MODE_IN_CALL else AudioManager.MODE_NORMAL
        audioManager.isSpeakerphoneOn = !isEarpieceMode
        val streamType =
            if (isEarpieceMode) AudioManager.STREAM_VOICE_CALL else AudioManager.STREAM_MUSIC
...
//        window?.clearFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON)
    ...
    fun play(
        file: File?,
        msgKey: String?,
        listenerMsg: VoiceMsgPlayEventListener?,
        isReplay: Boolean
    ) {
...
//        isEarpieceMode = SPStroage.earpieceMode//YJ
        ...
        mediaPlayer.setWakeMode(mContext, PowerManager.PROXIMITY_SCREEN_OFF_WAKE_LOCK)
