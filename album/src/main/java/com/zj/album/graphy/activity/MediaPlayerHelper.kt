 ...
 * Filename: NewMediaPlayerHepler
 *
 * Created by yzf on 2018/4/26.
...
class MediaPlayerHelper {
    ...
    private val trackSelector: DefaultTrackSelector = DefaultTrackSelector(AdaptiveTrackSelection.Factory(bandwidthMeter))

    private lateinit var player: SimpleExoPlayer

    private lateinit var listener: VideoPlayEventListener

    private val vm = VoicePlayManager
    ...
    private fun postProgress() {
        ...
        timer?.scheduleAtFixedRate(object : TimerTask() {
            override fun run() {
                ...
                handler.post { listener.onProgress(player.currentPosition.toInt(), player.duration.toInt()) }
            }
        }, 0, 500)
    }
